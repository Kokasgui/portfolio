    ...
    for (const [key, value] of Object.entries(object)) {
        ...
        if (savedValue !== null) {
            ...
            if (input.getAttribute("type") === "checkbox") {
                input.setAttribute("checked", savedValue);
            }
        } else {
            ...
            if (input.getAttribute("type") === "range") {
                ...
                console.log(input.parentElement.querySelector("output").getAttribute("value"));
                input.parentElement.querySelector("output").innerText = `${(accessOptions[group][key] / 1000).toFixed(2)} s`;
            }
            if (input.getAttribute("type") === "checkbox") {
                input.setAttribute("checked", accessOptions[group][key]);
            }
        }
    }
}
...
document.querySelectorAll("#options input").forEach((input) => {
    ...
    input.addEventListener("change", () => {
        ...
        if (input.getAttribute("type") === "checkbox") {
            if (input.hasAttribute("checked") !== accessOptions[group][key]) {
                accessOptions[group][key] = input.hasAttribute("checked");
            }
        }
...
function ChangeGallerySlide(direction) {
    ...
    // galleryButton.onmousedown = (e) => {
    //     mouseDown = true;
    // };
    // galleryButton.onmouseup = (e) => {
    //     mouseDown = false;
    // };
...
function PlayAnimation(element) {
    ...
    if (element.tagName === "VIDEO" && element.played) {
        element.play();
    }
}
...
const actionbarLabels = {
    ...
    inputActivate: "Ativar",
    inputDeactivate: "Desativar",
...
function UpdateActionBar(gamepadType) {
    // CRIAR LABELS DE INÍCIO, EM VEZ DE TERMOS DOIS LOGO NO FICHEIRO HTML?
        ...
        switch (i) {
            case 0:
                ...
                if (
                    ...
                    (focusedElement.tagName === "INPUT" && focusedElement.getAttribute("type") === "checkbox") ||
                    (focusedElement.tagName === "LABEL" && focusedElement.parentElement.querySelector("input[type='checkbox']"))
                ) {
                    let checkbox;
            ...
            case 1:
                ...
                } else if (focusedElement.tagName === "SECTION" || focusedElement.tagName === "FOOTER") {
            ...
            case 2:
                ...
                if (
                    (focusedElement.tagName === "INPUT" && focusedElement.getAttribute("type") === "range") ||
                    (focusedElement.tagName === "LABEL" && focusedElement.parentElement.querySelector("input[type='range']"))
                ) {
                    // console.log("ISTO É UM INPUT DE RANGE");
                    imgName = "inputSlide";
                    action = "inputSlide";
                } else if (heroHome && heroHome.contains(focusedElement)) {
        ...
        if (
            ...
            if (!currentKeys.includes("Tab")) {
                if (focusedElement === document.body) {
                    if (e.key === "Backspace" || e.key === "Delete" || e.key === "Escape" || e.key === "Esc") {
                ...
                //     // if (document.querySelector("#options").getAttribute("aria-hidden") === "true") ToggleNavMenu("keyboard");
                ...
                if (!form || (form && !writingMode) || (form && !form.contains(focusedElement))) {
                    ...
                    if (focusedElement === document.body) {
                        ...
                        ) // Se alguma das teclas de navegação for acionada, mas não estiver
                        {
                            ...
                            if (navButton.getAttribute("aria-expanded") === "false") main.querySelector("section").focus();
                            else if (
                    ...
                    if (e.key === "e" || e.key === "E" || e.key === "p" || e.key === "P") {
                        ...
                        // Repõe o atalho de teclado para avançar no histórico
                        if (heroHome && heroHome.contains(focusedElement)) {
...
function MoveSelection(direction, columnOrRow) {
    ...
    // if (focusedElement.tagName === "SECTION" || focusedElement === footer) {
    //     const notFocusVisibleArray = Array.from(document.querySelectorAll("section:not(:focus-visible), footer:not(:focus-visible)"));
    //     console.log(notFocusVisibleArray);
    //     if (notFocusVisibleArray.includes(focusedElement)) {
    //         // alert("yo");
    //         // focusedElement.focus();
    //         // return;
    //     } else {
    ...
    if (main.contains(focusedElement) || footer.contains(focusedElement) || mainNav.contains(focusedElement) || options.contains(focusedElement)) {
        ...
        while (currentNode) {
            ...
            if (columnOrRow === "row" && options.contains(currentNode) && (currentNode.tagName === "LABEL" || currentNode.tagName === "INPUT")) {
                // debugger;
                ...
                if (input) {
                    if (input.getAttribute("type") === "range") {
                        ...
                        // input.parentElement.querySelector("output").innerText = accessOptions[name];
...
function MainLoop() {
    ...
    // console.log(focusedElement, window.getComputedStyle(focusedElement).backgroundImage);
    ...
    //         if (input.valueAsNumber !== accessOptions[group][key]) {
    //             accessOptions[group][key] = input.valueAsNumber;
    //             // input.setAttribute("aria-valuenow", input.getAttribute("value"));
    //             input.parentElement.querySelector("output").innerText = `${(accessOptions[group][key] / 1000).toFixed(2)} s`;
    //         }
    //     }
    //     if (input.getAttribute("type") === "checkbox") {
    //         if (input.hasAttribute("checked") !== accessOptions[group][key]) {
    //             accessOptions[group][key] = input.hasAttribute("checked");
    //             // input.setAttribute("aria-checked", input.hasAttribute("checked"));
    ...
    // if (document.querySelector("#main-nav").contains(focusedElement) && navButton.getAttribute("aria-expanded")=== "true") {
    ...
    // document.addEventListener("mousedown", (e) => {
    //     if (lastFocusedElement.tagName === "SECTION") {
    //         focusedElement.blur();
    //     }
    //     mouseDown = true;
    // });
