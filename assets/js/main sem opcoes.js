...
const accessOptions = {
    ...
    highContrast: false,
    controller: {
        holdDelay: 450, // 450, 600, 750, 900 ms
...
document.addEventListener("keydown", (e) => {
    ...
    function KeyActions() {
        ...
        if (focusedElement === document.body && lastFocusedElement === document.body) {
            if (!currentKeys.includes("Escape")) {
                e.preventDefault();
        ...
        if (e.key === "Backspace" || e.key === "Delete") {
            if (document.querySelector("#main-nav").contains(focusedElement)) ToggleNavMenu("keyboard");
            else if (
...
function MoveSelection(direction, columnOrRow) {
    ...
    if (main.contains(focusedElement) || footer.contains(focusedElement) || navList.contains(focusedElement)) {
        ...
        while (currentNode) {
            ...
            if (main.contains(currentNode) || footer.contains(currentNode) || navList.contains(currentNode))
                currentNode = parent;
            else return;
        }
    } else {
        ...
        if (gamepad.mapping === "standard") {
            ...
            const buttonPressActions = {
                ...
                1: () => {
                    ...
                    if (document.querySelector("#main-nav").contains(focusedElement)) ToggleNavMenu("gamepad");
